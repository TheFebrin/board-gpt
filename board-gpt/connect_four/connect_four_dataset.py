...
class CharConnectFourDataset(Dataset):
    ...
    def __init__(self, cf_dataset: ConnectFourDataset):
        self.chars = sorted(
            list(set(list(itertools.chain.from_iterable(cf_dataset))))
            + [
                -100,
            ]
        )
        data_size, vocab_size = len(cf_dataset), len(
            self.chars
        )  # vocab size 61, with -100 sorted to the front
        max_len = max(
            [len(cf_dataset[i]) for i in range(len(cf_dataset))]
        )  # should be 60 in Othello
        print(
            "Dataset created has %d sequences, %d unique words."
