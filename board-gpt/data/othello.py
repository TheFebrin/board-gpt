...
class Othello:
    def __init__(self, ood_perc=0., data_root=None, wthor=False, ood_num=1000, train_size=800):
        ...
        # load simulated games from files. Otheriwse, generate ood_num games.
        ...
        if data_root is None:
            ...
            if ood_num == 0:
                print("Syntethic dataset size was set to 0 - returning.")
                return
            else:
                if ood_num != -1:  # this setting used for generating synthetic dataset
                    print("ood_num has a value different than -1.")
                    ...
                    for can in tqdm(p.imap(get_ood_game, range(ood_num)), total=ood_num):
                        # This actually fails to generate exactly ood_num games. If some game have been generated
                        # for the second time, it won't be added to the dataset and therefore the dataset will be smaller
                        # than the ood_num value.
                        print("Newly generated game: {}".format(can))
                        print("Game size: {}".format(len(can)))
                        if not can in self.sequences:
                            print("Game is not present in sequences. Appending...")
                            self.sequences.append(can)
                        else:
                            print("Game is already in the sequences.")
                    p.close()
                ...
                self.val = self.sequences[train_size:]
                self.sequences = self.sequences[:train_size]
                print(f"Using {train_size} for training, {len(self.val)} for validation")
        else:
            for fn in os.listdir(data_root):
                if criteria(fn):
                    with open(os.path.join(data_root, fn), "r", encoding="utf-8") as f:
                        pgn_text = f.read()
...
def get_ood_game(_):
    """
    Generates a random Othello game.
    TODO: Figure out why all randomly generated games consist of exactly 60 moves.
    """
    tbr = []
...
def get(ood_perc=0., data_root=None, wthor=False, ood_num=1000, train_size=800):
    return Othello(ood_perc, data_root, wthor, ood_num, train_size)
